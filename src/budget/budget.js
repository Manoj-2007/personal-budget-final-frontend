    ...
    const handleDeleteBudget = async () => {
        try {
            ...
            const response = await axios.delete(
                `http://167.99.61.114:4219/api/delete-budget/${budgetToDelete.id}`,
                {
                    headers: {
    ...
    const handleAddBudget = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://167.99.61.114:4219/api/add-budget',
                newBudget,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (error.response.status === 401) {
                ...
                if(error.response.data.error.includes('Duplicate Entry')){
                    alert(`Duplicate Entry for Budget`);
    ...
    useEffect(() => {
        const fetchBudgets = async () => {
            try {
                ...
                const response = await axios.get('http://167.99.61.114:4219/api/budgets', {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
                ...
                <div className="container">
                    <div className="row">
                        ...
                        <div className='col-md-6 text-right'>
                            <Button style={{backgroundColor : 'green', color: 'white'}} variant={'secondary'} onClick={handleShow}>
                                Add A Budget
                            </Button>
                        </div>
                    </div>
